    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">{t('img_heatmap_title')}</span>
          <span className="text-xs text-muted-foreground">{t('img_heatmap_subtitle')}</span>
        </div>
        <button
          onClick={() => setShowHeatmap(v => !v)}
          className="text-xs text-primary hover:underline flex items-center gap-1"
        >
          {showHeatmap ? t('hide') : t('show')} {t('overlay')}
        ...
        <div className="flex flex-wrap gap-2">
          {regions.map((r, i) => (
            <div key={i} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs border ${r.intensity > 0.6 ? 'bg-rose-400/10 border-rose-400/30 text-rose-400' : 'bg-amber-400/10 border-amber-400/30 text-amber-400'}`}>
              ...
              {r.label} — {Math.round(r.intensity * 100)}% {t('suspicious')}
            </div>
          ))}
        </div>
      )}
...
function ShareButton({ result, file }: { result: DetectionResult | null; file: File | null }) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({ title: 'FakeGuard Detection Report', url: shareUrl });
      } else {
  ...
  return (
    ...
    >
      ...
      {isSharing ? (t('creating_link')) : (t('share_report'))}
    </Button>
  );
}
...
export default function ImageDetect() {
  ...
  useEffect(() => {
    document.title = t('img_page_title') + ' — FakeGuard';
  }, []);
  ...
  const handleDownloadReport = () => {
    if (!result) return;
    const report = [
      '=== FakeGuard Forensic Analysis Report ===',
      ...
      '',
      '--- MANIPULATED REGIONS ---',
      ...result.heatmapRegions.map(r => `• ${r.label}: ${Math.round(r.intensity * 100)}% {t('suspicious')}`),
      '',
      '=== END OF REPORT ===',
      'This report is generated by FakeGuard AI Detection Platform.',
            ...
            <div className="space-y-4">
                  ...
                  {isAnalyzing && (
                    <div className="mt-3">
                      <div className="flex justify-between text-xs text-muted-foreground mb-1">
                        <span>{uploadProgress < 75 ? t('uploading') : t('analyzing_with_ai')}</span>
              ...
              {!result && (
                <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full bg-violet-500 hover:bg-violet-600 text-white gap-2">
                  {isAnalyzing ? (
                    <><Cpu className="w-4 h-4 animate-spin" /> {t('analyzing_image')}</>
                  ) : (
                    <><Search className="w-4 h-4" /> {t('analyze_for_deepfakes')}</>
                  )}
                </Button>
              )}
            </div>
            ...
            <div className={`rounded-2xl border ${colors.border} bg-card p-6`}>
              <div className="flex items-start justify-between gap-4 mb-5">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h2 className="text-lg font-bold text-foreground">
                      {result.verdict === 'deepfake' ? t('verdict_deepfake_image') :
                        result.verdict === 'suspicious' ? t('verdict_suspicious') :
                          t('verdict_safe')}
                    </h2>
                  </div>
                ...
                <div className="text-center flex-shrink-0">
                  ...
                  <div className="text-xs text-muted-foreground">{t('risk_score')}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="text-center p-3 rounded-xl bg-muted/30">
                  ...
                  <div className="text-xs text-muted-foreground">{t('ai_model')}</div>
                </div>
              </div>
              ...
              {result.engineBreakdown && result.engineBreakdown.length > 0 && (
                <div className="mt-4 pt-4 border-t border-border/40">
                  <p className="text-xs text-muted-foreground mb-3 font-medium uppercase tracking-wider flex items-center gap-1.5">
                    <Layers className="w-3.5 h-3.5" /> {t('multi_engine_verification')}
                  </p>
                      ...
                      <div key={eng.engine} className="flex items-center gap-3">
                        ...
                        {eng.available ? (
                          <>
                            ...
                            <div className="text-xs text-muted-foreground w-12 text-right flex-shrink-0">{Math.round(eng.weight * 100)}% {t('weight_abbr')}</div>
                          </>
                        ) : (
                          <div className="flex-1 text-xs text-muted-foreground/50 italic">{t('engine_not_configured')}</div>
                        )}
                      </div>
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground/60 mt-2">
{t('engine_weighted_avg')}
                    {result.engineBreakdown.filter(e => !e.available).length > 0 && <span className="text-yellow-500/60 ml-1">({t('some_engines_unavailable')})</span>}
                  </p>
                </div>
              )}
              ...
              {result.possibleSources.length > 0 && (
                <div className="mt-4 pt-4 border-t border-border/40">
                  <p className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wider">{t('possible_ai_sources')}</p>
            ...
            <div className="flex gap-1 p-1 rounded-xl bg-muted/30 border border-border/40">
              {[
                { id: 'heatmap', label: t('tab_heatmap'), icon: Eye },
                { id: 'forensic', label: t('tab_forensics'), icon: Hash },
                { id: 'report', label: t('tab_report'), icon: BarChart3 },
              ].map(tab => (
                <button
                  key={tab.id}
              ...
              {activeTab === 'forensic' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="font-semibold text-foreground">{t('file_forensic_analysis')}</span>
                    <span className="text-xs text-muted-foreground">{t('exif_subtitle')}</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[
                      { label: t('forensic_format'), value: result.forensic.format, icon: FileImage },
                      { label: t('forensic_dimensions'), value: result.forensic.dimensions, icon: Camera },
                      { label: t('forensic_colorspace'), value: result.forensic.colorSpace, icon: Layers },
                      { label: t('forensic_software'), value: result.forensic.software, icon: Cpu },
                      { label: t('forensic_exif'), value: result.forensic.hasExif ? t('exif_present') : t('exif_missing'), icon: Info },
                      { label: t('forensic_camera'), value: result.forensic.cameraModel || t('not_found'), icon: Camera },
                      { label: t('forensic_date'), value: result.forensic.creationDate || t('not_found'), icon: Clock },
                      { label: t('forensic_gps'), value: result.forensic.gpsData || t('not_found'), icon: Search },
                    ].map((item, i) => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
                    {[
                      { label: t('forensic_metadata'), value: result.forensic.metadataIntegrity, good: result.forensic.metadataIntegrity === 'Consistent' },
                      { label: t('forensic_noise'), value: result.forensic.noisePattern, good: result.forensic.noisePattern === 'Natural' },
                      { label: t('forensic_compression'), value: result.forensic.compressionArtifacts, good: result.forensic.compressionArtifacts === 'Normal' },
                    ].map((item, i) => (
              ...
              {activeTab === 'report' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="font-semibold text-foreground">{t('detection_feature_analysis')}</span>
                  </div>
                  <div className="space-y-3">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <Button onClick={() => { setFile(null); setPreviewUrl(null); setResult(null); }} variant="outline" className="flex-1 border-border/60 hover:border-primary/40 gap-2 min-w-[120px]">
                <FileImage className="w-4 h-4" /> {t('analyze_another')}
              </Button>
            </div>
            ...
            <div className="p-4 rounded-xl border border-violet-500/20 bg-violet-500/5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-medium text-violet-400 mb-0.5">{t('upgrade_court_reports')}</p>
                  <p className="text-xs text-muted-foreground">{t('upgrade_court_desc')}</p>
                </div>
              </div>
            </div>
